...
export class JspmCdnPaths {
    ...
    private printResults(results: JspmPathsLists): void {
        const logger = LoggerInstance.withType("JSPM");
        if (results.Resolved.length > 0) {
            logger.info(
                [
                    `Successfully resolved ${results.Resolved.length} path${
                        results.Resolved.length > 1 ? "s" : ""
                    }:`
                ]
                    .concat(
        ...
        if (results.Unresolved.length > 0) {
            logger.warn(
                [
                    `Failed to resolved ${results.Unresolved.length} path${
                        results.Unresolved.length > 1 ? "s" : ""
                    }:`
                ]
                    .concat(
                        results.Unresolved.map(
                            (item, index) =>
                                `${this.resultPrefix(
                                    index,
    ...
    private async saveResultToFile(paths: JspmPaths): Promise<{}> {
        ...
        return new Promise(resolve => {
            const pathname = [
                Paths.Directories.Source,
                "configs",
                "jspm.config.production.js"
            ].join("/");
        ...
        return new Promise<JspmPathsLists>(async resolve => {
            if (packagesList.length > 0) {
                ...
                if (cdnLink !== undefined) {
                    ...
                    results.Paths[item.FullName] = cdnLink.replace(
                        /^https?\:/i,
                        ""
                    );
                } else {
    ...
    private async getCdnLink(
        packageItem: PackageItem,
        splited: boolean = false
    ): Promise<string | undefined> {
        return new Promise<string | undefined>(resolve => {
            const logger = LoggerInstance.withType(
                `JSPM [${packageItem.FullName}]`
            );
            ...
            const request = https.get(location, response => {
                ...
                response.on("end", async () => {
                    ...
                    try {
                        ...
                        logger.error(
                            `Sorry, something wrong with response data from ${
                                CDN_API.Hostname
                            }. Please report a bug.`
                        );
                    }
                    ...
                    if (parsedJson !== undefined) {
                        logger.info(
                            `Downloaded and parsed '${
                                parsedJson.total
                            }' result${parsedJson.total > 1 ? "s" : ""}.`
                        );
                        if (parsedJson.total > 0) {
                            const link = await this.getLinkFromResponseByVersion(
                                packageItem,
                                parsedJson,
                                splited
                            );
                            if (link !== undefined) {
                                ...
                                logger.warn(
                                    `Cannot resolve cdn link with version ${
                                        packageItem.Details.Version
                                    }.`
                                );
                            }
                            resolve(link);
                            ...
                            if (!splited) {
                                const splitedName = this.tryToSplitPackageName(
                                    packageItem.Details.Name
                                );
                                if (splitedName !== undefined) {
                                    logger.info(
                                        `Trying to use splited package name: '${splitedName}'`
                                    );
    ...
    private tryToResolveSplitedPackage(
        ...
        const originalName = (
            packageItem.Details.OriginalName || packageItem.Details.Name
        ).toLowerCase();
        // Searching .min.js or .js by order priority
        searchingFiles.push(`${originalName}.min.js`);
        searchingFiles.push(`${originalName}.js`);

        const foundFile = searchingFiles.find(
            searchingFile =>
                asset.files.findIndex(file => searchingFile === file) !== -1
        );
        if (foundFile !== undefined) {
            logger.info(
                `File '${foundFile}' found in '${found.name}@${asset.version}'`
            );
            return this.buildCdnLinkWithCustomFile(
                link || found.latest,
                foundFile
            );
        }
    ...
    private async getLinkFromResponseByVersion(
        ...
        return new Promise<string | undefined>(async resolve => {
            const found = responseDto.results.find(
                x => x.name === packageItem.Details.Name
            );
            if (found !== undefined) {
                if (found.version === packageItem.Details.Version) {
                    ...
                    if (!splited) {
                        ...
                        const foundAsset = found.assets.findIndex(
                            x => x.version === packageItem.Details.Version
                        );
                        if (foundAsset !== -1) {
                            resolve(
                                this.tryToResolveSplitedPackage(
                                    packageItem,
                                    foundAsset,
                                    found
                                )
                    ...
                    logger.info(
                        `Not targeting the latest version '${
                            found.version
                        }', trying to find version '${
                            packageItem.Details.Version
                        }'`
                    );
                    const assetIndex = found.assets.findIndex(
                        x => x.version === packageItem.Details.Version
                    );
                    if (assetIndex !== -1) {
                        const asset = found.assets[assetIndex];
                        logger.info(
                            `Version '${packageItem.Details.Version}' found.`
                        );
                        const link = this.buildCdnLink(
                            found.latest,
                            found.version,
                            packageItem.Details.Version
                        );
                        if (link !== undefined) {
                            if (splited) {
                                ...
                                let results = this.checkIfFileExist(
                                    asset.files,
                                    link,
                                    checkFile
                                );
                ...
                logger.info(
                    `Package '${
                        packageItem.Details.Name
                    }' with original name was not found. Searching file in assets.`
                );
                let resolved = false;

                for (let i = 0; i < responseDto.results.length; i++) {
                    const item = responseDto.results[i];
                    const foundAsset = item.assets.findIndex(
                        x => x.version === packageItem.Details.Version
                    );
                    if (foundAsset !== -1) {
                        const resolvedItem = this.tryToResolveSplitedPackage(
                            packageItem,
                            foundAsset,
                            item
                        );
    ...
    private buildCdnLinkWithCustomFile(
        latestLink: string,
        targetFile: string
    ): string {
            ...
            if (packagesMap.hasOwnProperty(name)) {
                ...
                if (fullName.indexOf(prefix) === 0) {
                    const details = fullName
                        .slice(prefix.length, fullName.length)
                        .split("@");
                    packagesList.push({
                        MapName: name,
                        FullName: fullName,
